...
from test_personas_from_audio import accept, client, make_persona, upload


def create_via_draft(c, filename, **overrides):
    r = upload(c, filename)
    assert r.status_code == 200
    r = accept(c, r.json()["token"], **overrides)
    assert r.status_code == 200
    return r
...
def test_retranscribe_happy(client):
    c, asr, _, tmp = client
    r = create_via_draft(c, "Tifa_Eng.wav")
...
def test_retranscribe_pasa_idioma(client):
    ...
    assert r.status_code == 200
    r = accept(c, r.json()["token"], name="Zack")
    assert r.status_code == 200
...
def test_retranscribe_502(client):
    c, asr, _, tmp = client
    create_via_draft(c, "Tifa_Eng.wav")
...
def test_get_unico_incluye_transcript(client):
    ...
    assert r.status_code == 200
    r = accept(c, r.json()["token"], transcript=asr.text)
    assert r.status_code == 200
